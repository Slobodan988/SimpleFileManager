...
public class ColorPreference {
    ...
    private static int getColorByIndex(int colorIndex) {
        ...
        if(colorIndex < 0 || colorIndex > availableColors.size() -1)
        {
    ...
    public static ColorPreference loadFromPreferences(Context context, SharedPreferences preferences) {
        ...
        for (ColorUsage usage : ColorUsage.values()) {
            try {
                ...
                Log.i("test_boja", "test log 2: " + val);
    ...
    public ColorPreference randomize() {
        ...
        int randInt = new Random().nextInt(combinations.length - 1);
        int[] colorPos = combinations[randInt];
        for(int i = 0; i< colorPos.length; i++)
        {
            Log.i("test_boja", "boja: " + i  + " " + Integer.toString(colorPos[i]));
        }
